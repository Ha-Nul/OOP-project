...
bool ATM::Allowed_cash(long long cash) {
	...
	if (result == false) {
		if (language == 1) {
			...
			wcout << "허용되지 않은 현금의 금액입니다" << endl;
			wcout << "오직 다음과 같은 현금 종류만 허용됩니다" << endl;
			wcout << "1000 원, 5000 원, 10000 원, 50000 원" << endl;
		}
	}
	return result;
}
...
bool ATM::Allowed_check(long long check) {
	...
	if (result == false) {
		if (language == 1) {
			...
			wcout << "허용되지 않은 수표의 금액입니다" << endl;
			wcout << "100000 원 이상의 수표만 허용됩니다" << endl;
		}
	}
	return result;
}
		...
		while (1) {
			...
			if (transfer_type == 1) {
				...
				if (language == 1) {
					...
					wcout << "계좌 이체가 선택되었습니다" << endl;
					wcout << "출금 계좌는 " << source_bank->Call_bank_name() << " " << _account_number << " 입니다" << endl;
				}
				break;
			}
			else {
		...
		while (1) {
			...
			if (Account_Exist(destination_account_number)) {
				if (language == 1) {
					...
					wcout << "입금 계좌 번호는 " << destination_account_number << " 입니다" << endl;
				}
				break;
			}
			else {
				if (language == 1) {
					...
					wcout << "입금 계좌 번호가 존재하지 않습니다" << endl;
				}
				continue;
			}
		}
		...
		if (transfer_type == 1) {
			...
			while (1) {
				if (language == 1) {
					...
					wcout << "현금 이체하실 금액에 현금 이체 수수료 " << cash_transfer_fee << " 원을 더하여 지불해주세요" << endl;
					wcout << "지불하실 현금의 금액를 입력해주세요" << endl;
				}
				cin >> pay;
				if (Allowed_cash(pay) == false) {
					...
					if (language == 1) {
						...
						wcout << "지불하신 금액이 현금 이체 수수료보다 작거나 같습니다" << endl;
					}
					continue;
				}
				else {
					if (language == 1) {
						...
						wcout << "지불하신 총 금액은 " << pay << " 원입니다" << endl;
						wcout << "현금 이체 금액: " << pay << " 원" << endl;
						wcout << "현금 이체 수수료: " << cash_transfer_fee << " 원" << endl;
					}
					availabe_cash += pay;
					break;
				}
			...
			if (language == 1) {
				...
				wcout << pay - cash_transfer_fee << " 원이 " << destination_bank->Call_bank_name() << " " << destination_account_number << " 으로 현금 이체 되었습니다" << endl;
			}
			return;
			/*=====================================================================*/
		}
		...
		if (transfer_type == 2) {
			...
			while (1) {
				if (language == 1) {
					...
					wcout << "출금 계좌의 잔액: " << source_bank->Call_balance(_account_number) << " 원" << endl;
					wcout << "계좌 이체의 경우 계좌 이체 수수료 " << account_transfer_fee << " 원이 출금 계좌에서 추가로 지불됩니다" << endl;
					wcout << "송금하실 금액을 입력해주세요" << endl;
				}
				cin >> amount_transfer;
				if (source_bank->Call_balance(_account_number) < amount_transfer + account_transfer_fee) {
					...
					if (language == 1) {
						...
						wcout << "송금 금액: " << amount_transfer << " 원" << endl;
						wcout << "계좌 이체 수수료: " << account_transfer_fee << " 원" << endl;
					}
					break;
				}
			}
			...
			if (language == 1) {
				...
				wcout << amount_transfer << " 원이 " << source_bank->Call_bank_name() << " " << _account_number << " 에서 " << destination_bank->Call_bank_name() << " " << destination_account_number << " 으로 계좌 이체 되었습니다" << endl;
			}
			return;
			/*=====================================================================*/
